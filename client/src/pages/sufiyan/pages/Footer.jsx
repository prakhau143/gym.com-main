...
import { FaInstagram, FaTwitter, FaYoutube, FaFacebook, FaPhoneAlt, FaMapMarkerAlt, FaGlobe, FaCheckCircle, FaStar, FaClock, FaEnvelope } from 'react-icons/fa';
import { motion } from 'framer-motion';
import { ReactNode } from 'react';
import logo from "../../../assets/fit4you-logo.png"
...
const businessData = {
  name: "FIT 4 YOU",
  tagline: "Empowering Women. Inspiring Fitness.",
  description: "A women-only gym dedicated to empowering women of all ages and backgrounds to achieve their health and fitness goals in a supportive and inspiring environment.",
  address: "Fort Wayne, IN",
  phone: "+1 260-312-2565",
  email: "info@fit4you.com",
  website: "https://m.facebook.com/155587324503413/",
  socials: {
...
function InfoSection() {
  return (
    <Box
      ...
      mb={4}
      p={{ base: 4, md: 8 }}
      bgGradient="linear(to-r, orange.100, orange.300, pink.200)"
      borderRadius="2xl"
      boxShadow="2xl"
      initial={{ opacity: 0, y: 40 }}
      animate={{ opacity: 1, y: 0 }}
      transition={{ duration: 0.8 }}
      _dark={{ bgGradient: 'linear(to-r, gray.700, gray.800, black)' }}
      position="relative"
      overflow="hidden"
    >
      <Box
        ...
        right={-6}
        boxSize={32}
        bgGradient="radial(orange.200, transparent 70%)"
        zIndex={0}
        filter="blur(8px)"
      />
      <Stack spacing={3} align="center" position="relative" zIndex={1}>
        <Text fontWeight="bold" fontSize={{ base: '2xl', md: '3xl' }} color="#F45F02" letterSpacing="wide" fontFamily="'Exo', 'Poppins', sans-serif">
          {businessData.name}
        </Text>
        <Text fontSize={{ base: 'md', md: 'lg' }} color="gray.700" fontStyle="italic">
          {businessData.tagline}
        </Text>
        <Text fontSize={{ base: 'sm', md: 'md' }} color="gray.600" maxW="600px" textAlign="center">
          {businessData.description}
        </Text>
        <Divider my={2} />
        <Stack direction={{ base: 'column', md: 'row' }} spacing={8} justify="center" align="center">
          <HStack spacing={2}>
            <FaMapMarkerAlt color="#ED8936" size={20} />
            <Text color="gray.700" fontWeight="bold">{businessData.address}</Text>
          </HStack>
          <HStack spacing={2}>
            <FaPhoneAlt color="#38B2AC" size={20} />
            <Link href={`tel:${businessData.phone}`} color="teal.500" fontWeight="bold">{businessData.phone}</Link>
          </HStack>
          <HStack spacing={2}>
            <FaGlobe color="#ff6600" size={20} />
            <Link href={businessData.website} color="orange.500" fontWeight="bold" fontSize="md" isExternal>Website</Link>
          </HStack>
          <HStack spacing={2}>
            <FaEnvelope color="#F45F02" size={20} />
            <Link href={`mailto:${businessData.email}`} color="orange.600" fontWeight="bold">{businessData.email}</Link>
          </HStack>
        </Stack>
        <Stack direction="row" spacing={5} mt={2} justify="center">
          {businessData.socials.facebook && (
            <Link href={businessData.socials.facebook} isExternal>
              <FaFacebook color="#1877f3" size={24} />
            </Link>
          )}
          {businessData.socials.instagram && (
            <Link href={businessData.socials.instagram} isExternal>
              <FaInstagram color="#E1306C" size={24} />
            </Link>
          )}
          {businessData.socials.youtube && (
            <Link href={businessData.socials.youtube} isExternal>
    ...
    >
      ...
      >
        <Container
          ...
          justify={'center'}
          align={'center'}>
          <Text fontWeight="extrabold" fontSize={{base: '2xl', md: '3xl'}} letterSpacing="wide" color="#F45F02" fontFamily="'Exo', 'Poppins', sans-serif" textShadow="2px 2px 8px #222" py={2}>
            FIT 4 YOU
          </Text>
        ...
        <Box
          ...
          <Container
            ...
            <Text>© 2025 Prakhar. All rights reserved</Text>
            <Stack direction={'row'} spacing={6}>
...
export default FooterWithBusiness;